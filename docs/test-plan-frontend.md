...
> 时间限制通过房间配置「时间限制」设置（发言限时 / 任务超时 / 投票超时）。speechTimeout 已实现；roundTimeout / voteTimeout 仅配置存储，未强制实现（下述预期行为，当前测试会暴露未实现状态）。

- **F1** 配置发言限时（如 30 秒）后，discussion 阶段房主可见计时器卡片（timer-card）。
- **F2** 房主可对发言计时：开始 / 暂停 / 重置（`startTimer` / `pauseTimer` / `resetTimer`）。
- **F3** 发言倒计时归零后 toast「发言时间到」，计时自动停止。
- **F4** 未配置发言限时（不限）时，不显示计时器卡片、无计时功能（`hasSpeechTimeout` 为 false）。
- **F5** 时间限制配置保存后，房间配置 `roomConfig.limits` 正确下发（`state` 查看 limits.speechTimeout / roundTimeout / voteTimeout）。
- **F6** 【预期·未实现】任务超时（roundTimeout）：超时后未行动的任务队玩家自动投成功票；必须出失败票的角色超时仍出失败票。
- **F7** 【预期·未实现】投票超时（voteTimeout）：队伍投票超时后未行动玩家自动投赞成票。
- **F8** 发言限时 / 任务超时 / 投票超时支持独立或组合配置。

### G. 房间规则配置

> 各规则通过房间配置「基础规则 / 兰斯洛特配置 / 必败设置」配置，按需开启/关闭后开局验证。视野类用例需以对应角色（bot）查看「身份」视野。

- **G1** 流车上限更改：配置流车上限（maxFailedNominations=2/3/4/5），同轮流车数达上限后下一车为强制车；验证强制车按钮与流程按新上限触发。
- **G2** 坏人互知身份：开启时睁眼狼视野互认其他睁眼狼（除奥伯伦）；关闭时坏人互相不可见（各坏人不知谁是自己人）。
- **G3** 票型公开/匿名：`voteVisibility` 公开=队伍投票结束后显示各玩家赞成/反对座位号；匿名=仅显示赞成/反对聚合票数。
- **G4** 任务失败详情 票数/成败：`missionFailDetail` 票数=任务失败时显示失败票数；成败=仅显示成功/失败。
- **G5** 奥伯伦必须任务失败：开启时奥伯伦进任务队只能投失败（后端拒绝投成功，提示「你当前阵营为坏人，必须投失败票」）；关闭时可自由选择。
- **G6** 兰斯洛特必须任务失败：开启时当前为坏阵营的兰斯洛特（蓝/红兰转换后）只能投失败；关闭时可自由选择。
- **G7** 兰斯互认身份：开启时蓝兰/红兰视野互知对方身份；关闭时互不可见。
- **G8** 兰斯换身轮次：配置换身轮次（1-4），到达该轮触发兰斯抽卡（lancelot 阶段），转换/保持按抽卡结果。
- **G9** 睁眼狼知红兰：开启时睁眼狼（莫甘娜/刺客等）视野可见红兰身份；关闭时不可见。
- **G10** 奥伯伦知红兰：开启时奥伯伦视野可见红兰身份；关闭时不可见。
- **G11** 梅林辨兰斯阵营：开启时梅林视野可分辨蓝兰/红兰具体阵营；关闭时梅林只见双兰但不知谁蓝谁红。

### H. 观战

> 观战通过房间配置「观战设置」（允许观战 + 上限）控制。进入/退出观战为玩家操作（需 2+ 真机或开发者工具多窗口）。

- **H1** 观战：配置允许观战且设上限时，可进入观战区；观战区显示观战玩家、ID 卡片观战计数（未开启时显示 -/-）。
- **H2** 观战中途退出：观战中退出房间，观战区人数减少。
- **H3** 观战中途加入：游戏进行中，新玩家只能以观战者加入（自动落座 -1）；观战区未满可加入，已满则加入失败（提示「游戏已开始且观战区已满」）。
...
A1 A2 A3 A5 A6 B1 B2 B3 C1 C2 C3 D1 D2 D3 F4

### 5.2 测试流程（覆盖全部用例）

| 流程 | 场景 | 前置条件 | 覆盖用例 |
|------|------|---------|---------|
| 流程1 | 标准局·好人胜（三好车 + 刺错梅林） | 5人基准配置（G3 公开 / G4 票数） | 通用 + B4 C4 D4 J1 J4 G3 G4 |
| 流程2 | 坏人胜（三坏车直接获胜） | 5人基准配置 | 通用 + J5 |
| 流程3 | 流车 → 强制车 | 5人局，流车上限默认 3 | 通用 + B4 J2 J3 |
| 流程4 | 刺客主动开刀 | 5人基准配置 | 通用 + E1 E2 E3 |
| 流程5 | 湖仙配置局 | ≥10人局 + 湖仙开启 | A4 J6 |
| 流程6 | 兰斯配置局 | 11/12人局 + 兰斯配置 | J7 |
| 流程7 | 计时器配置局 | 配置发言限时 | B5 F1 F2 F3 |
| 流程8 | 规则组合局·坏人必败 | 含奥伯伦 + 「奥伯伦必须任务失败」开启 | 通用 + G5 |
| 流程9 | 规则组合局·兰斯必败 | 含兰斯洛特 + 「兰斯洛特必须任务失败」开启（转换后坏阵营） | 通用 + G6 |
| 流程10 | 规则组合局·兰斯视野 | 11/12人局 + 兰斯互认 / 换身轮次 / 睁眼狼知红兰 / 奥伯伦知红兰 / 梅林辨兰斯阵营 | 通用 + G7 G8 G9 G10 G11 |
| 流程11 | 观战局 | 配置允许观战（设上限 1）+ 5人局 | 通用 + H1 H2 H3 |

> 说明：
> - 通用用例不覆盖 G/H 各规则用例，均需在对应「前置条件」的配置局中验证。
> - G1（流车上限更改）可在流程3中另设流车上限为 2 复测。
> - G2（坏人互知身份）可在流程1/2 中分别开关验证。
> - H1-H3 需 2+ 玩家（真机或开发者工具多窗口）配合进入/退出观战区。
> - E4 / F6 / F7（多次开刀 / 任务超时 / 投票超时）为未实现，暂不列入覆盖。