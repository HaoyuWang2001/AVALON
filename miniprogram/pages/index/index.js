...
Page({
  ...
  // ─── 胜率冠军 ───
  loadChampions(openId) {
    api.getChampions(openId).then(res => {
      if (res && res.success && Array.isArray(res.champions)) {
        const champions = res.champions.map((c, i) => ({ ...c, rank: i + 1 }));
        const podium = [];
  ...
  goToGlobalStats() {
    wx.navigateTo({ url: '/pages/global-stats/global-stats' });
  },
